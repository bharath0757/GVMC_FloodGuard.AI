...
import {
  ShieldAlert, AlertTriangle, Building2, Camera,
  Brain, Navigation
} from 'lucide-react';
import { Badge, Card, CardContent, CardHeader, CardTitle, StatisticsCard } from '@floodguard/ui';
...
export const CommandCenterTab: React.FC = () => {
  ...
  const criticalWards = (aiSummary?.ward_predictions || []).filter((w) => w.risk_category === 'Critical');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-slate-800 bg-slate-900/60">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div>
                <CardTitle className="text-base text-white">XGBoost Flood Risk & Active High-Risk Wards</CardTitle>
                <span className="text-xs font-mono text-slate-400">15 Visakhapatnam Wards • Continuous Sensor Ingestion</span>
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(aiSummary?.ward_predictions || []).slice(0, 6).map((ward) => (
                  ...
                  >
                    <div>
                      <div className="font-bold text-white text-sm">{ward.ward_name}</div>
                      <div className="text-[10px] text-slate-400">Ward #{ward.ward_number} • {ward.rainfall_mm_hr} mm/h</div>
                    </div>
                    <div className="text-right">
                      <div className="text-base font-extrabold font-mono">{ward.risk_score.toFixed(0)}</div>
          ...
          <Card className="border-teal-500/30 bg-teal-950/10">
            <CardHeader className="pb-2">
              <div className="flex items-center space-x-2">
                ...
                <CardTitle className="text-base text-white font-mono uppercase">AI Command Center Recommendations</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="space-y-2 text-xs font-mono">
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-start space-x-3">
                ...
                <div>
                  <div className="font-bold text-white">Pre-position NDRF Boat Units in Gajuwaka (Ward #14)</div>
                  <p className="text-slate-400 text-[11px] mt-0.5">XGBoost risk score (88.2) combined with 68.2mm/h rainfall indicates high underpass inundation risk.</p>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-start space-x-3">
                ...
                <div>
                  <div className="font-bold text-white">Activate Secondary Relief Shelter AU Sports Complex</div>
                  <p className="text-slate-400 text-[11px] mt-0.5">Gajuwaka Stadium occupancy reached 79%. Divert new evacuees to AU Sports Complex via A* safe route.</p>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-start space-x-3">
                ...
                <div>
                  <div className="font-bold text-white">Issue Traffic Advisory for Submerged NH-16 Intersection</div>
                  <p className="text-slate-400 text-[11px] mt-0.5">Citizen report rep-101 verified water depth at 65cm. Redirect traffic toward Sheela Nagar arterial bypass.</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <Card className="border-slate-800 bg-slate-900/60 h-full">
            <CardHeader className="pb-2">
              <CardTitle className="text-base text-white">Emergency Timeline</CardTitle>
            </CardHeader>
            <CardContent>
              <IncidentTimeline />
            </CardContent>
