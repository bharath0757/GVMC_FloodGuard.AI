...
export interface SimulationState {
  ...
  /** Overflow hotspots */
  overflowPoints: { lat: number; lng: number; radius: number; intensity: number }[];
  /** Road flood segments */
  roadFloodZones: { lat: number; lng: number; width: number; intensity: number }[];
  /** Speed multiplier */
  speed: 0.5 | 1 | 2;
  isPlaying: boolean;
...
const WARD_CENTROIDS: Record<number, { lat: number; lng: number; bounds: [number, number][] }> = {
...
export function initSimulation(
  mockWards: Array<{ id: string; number: number; name: string; riskScore: number }>,
  rainfallMmHr = 68
): SimulationState {
...
export function wardFillColor(fillLevel: number): string {
  ...
  if (fillLevel < 0.40) return 'rgba(255,235,59,0.30)';  // yellow
