...
// Ward Risk Card (mini)
// ─────────────────────────────────────────────
const WardRiskCard: React.FC<{ ward: WardPrediction; onClick: () => void }> = ({
  ward,
  onClick,
}) => {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={`w-full rounded-xl border p-3 text-left backdrop-blur-sm transition-all ${ALERT_BG[ward.alert_color]}`}
    >
      ...
      <div className="mt-1.5 flex items-center justify-between font-mono text-[10px] opacity-70">
...
const PredictionPanel: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <span className="font-mono text-xs text-slate-400">
            Ward #{ward.ward_number} • Live AI Analysis
          </span>
        </div>
        <button
          onClick={onClose}
          className="p-1 text-xl text-slate-400 hover:text-white"
        >
          ✕
        </button>
      </div>
      ...
      {/* Risk Score */}
      <div className={`rounded-xl border p-4 ${ALERT_BG[ward.alert_color]}`}>
        <div className="mb-2 flex items-center justify-between">
          <span className="font-mono text-xs font-bold uppercase">
            AI Risk Score
          </span>
          <Badge
            variant={ward.alert_color === 'Red' ? 'destructive' : 'secondary'}
          >
            {ward.risk_category}
          </Badge>
        </div>
        ...
        <div className="mt-2 font-mono text-xs text-slate-300">
          Confidence: {(ward.confidence * 100).toFixed(1)}% • Model:{' '}
          {ward.model_used}
        </div>
      </div>
      ...
      {pred?.explanations && (
        <div>
          <div className="mb-2 font-mono text-xs font-bold uppercase text-slate-400">
          ...
          <div className="space-y-2">
            {pred.explanations.slice(0, 4).map((exp, i) => (
              ...
              >
                ...
                <div className="mx-3 h-1.5 flex-1 overflow-hidden rounded-full bg-slate-800">
                  <div
                    className={`h-full rounded-full ${exp.direction === 'increases_risk' ? 'bg-red-500' : 'bg-emerald-500'}`}
                    style={{
                      width: `${Math.min(100, Math.abs(exp.contribution))}%`,
                    }}
                  />
                </div>
                <span
      ...
      {shelter?.primary_shelter && (
        <div className="rounded-xl border border-emerald-800/50 bg-emerald-950/30 p-3">
          ...
          <div className="mt-1 flex items-center justify-between font-mono text-xs text-slate-300">
            <span>📍 {shelter.primary_shelter.distance_km} km away</span>
            <span>
              🏠 {shelter.primary_shelter.available_capacity} spaces free
            </span>
      ...
      {/* Evacuation Route */}
      {route?.primary_route && (
        <div className="rounded-xl border border-cyan-800/50 bg-cyan-950/30 p-3">
          <div className="mb-2 flex items-center space-x-2">
          ...
          <div className="flex flex-wrap items-center gap-1 space-x-1 font-mono text-xs text-slate-300">
            {route.primary_route.path.map((node, i) => (
              <React.Fragment key={node}>
                <span className="rounded bg-slate-800 px-1.5 py-0.5">
                  {node}
                </span>
                {i < route.primary_route.path.length - 1 && (
...
export const AIDashboard: React.FC = () => {
  ...
  const [selectedWard, setSelectedWard] = React.useState<WardPrediction | null>(
    null,
  );
      ...
      <div className="flex h-64 items-center justify-center text-slate-400">
        <div className="flex flex-col items-center space-y-3">
          ...
          <span className="font-mono text-xs">
            XGBoost Inference Engine Loading...
          </span>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="rounded-xl border border-teal-500/30 bg-teal-500/10 p-2">
            <Brain className="h-5 w-5 text-teal-400" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-white">
              AI Flood Intelligence Engine
            </h2>
            <span className="font-mono text-xs text-slate-400">
              XGBoost + A* Routing • 15 Wards Active • Visakhapatnam GVMC
            </span>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <div
            className={`flex items-center space-x-2 rounded-lg border px-3 py-1.5 font-mono text-xs font-bold ${alertColors[summary?.overall_alert_level || 'Green']}`}
          >
        ...
        {[
          {
            ...
            value: summary?.critical_wards ?? 0,
            icon: <AlertTriangle className="h-4 w-4" />,
            color: 'text-red-400 bg-red-500/10 border-red-500/30',
          },
          {
            label: 'High Risk Wards',
            value: summary?.high_risk_wards ?? 0,
          ...
          <motion.div
            ...
            className={`rounded-xl border p-4 ${m.color} flex flex-col space-y-2`}
          >
            <div className="flex items-center justify-between">
              {m.icon}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3">
          <div className="mb-3 flex items-center justify-between">
            <span className="font-mono text-xs font-bold uppercase text-slate-400">
              Ward Risk Scores (Click to Inspect)
            </span>
        ...
        {/* Detail Panel */}
        <div className="min-h-[480px] rounded-2xl border border-slate-800 bg-slate-900/60 p-4 lg:col-span-2">
          {selectedWard ? (
            <PredictionPanel
              ward={selectedWard}
            ...
            <div className="flex h-full flex-col items-center justify-center space-y-3 text-slate-400">
              ...
              <span className="text-center font-mono text-sm">
                Select a ward card to view
                <br />
                XGBoost predictions, SHAP explanations,
                <br />
                shelter recommendations & A* routes
              </span>
              <div className="flex animate-pulse items-center space-x-1.5 font-mono text-xs text-teal-400">
      ...
      {summary?.top_risk_ward && (
        <div
          className={`rounded-xl border p-4 ${ALERT_BG['Red']} flex items-center justify-between`}
        >
          <div className="flex items-center space-x-3">
            <AlertTriangle className="h-5 w-5 animate-pulse text-red-400" />
            <div>
              <span className="text-sm font-bold text-white">
                Highest Risk: {summary.top_risk_ward.ward_name} (Ward #
                {summary.top_risk_ward.ward_number})
              </span>
              <div className="mt-0.5 font-mono text-xs text-red-300">
                Risk Score: {summary.top_risk_ward.risk_score.toFixed(1)} | 🌧{' '}
                {summary.top_risk_ward.rainfall_mm_hr} mm/h | 💧{' '}
                {summary.top_risk_ward.water_level_cm} cm
              </div>
            </div>
          </div>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setSelectedWard(summary.top_risk_ward)}
          >
            Inspect →
          </Button>
        </div>
