...
from app.ai.services.ai_service import get_all_ward_predictions, WARD_TELEMETRY

# Visakhapatnam Ward Name Mapping
WARD_NAME_TO_NUM = {
    "gajuwaka": 14, "one town": 8, "maharanipeta": 3, "sheela nagar": 22,
    "seethammadhara": 11, "muralinagar": 16, "dwaraka nagar": 5,
    "gopalapatnam": 19, "mvp colony": 2, "old town": 1, "kancharapalem": 6,
    "akkayyapalem": 9, "pendurthi": 12, "madhurawada": 15, "kommadi": 20,
}
...
def process_assistant_query(query: str, user_lat: float = 17.6868, user_lng: float = 83.2185) -> Dict[str, Any]:
    """
    ...
    if any(k in q_lower for k in ["safe", "risk", "hazard", "water level", "flood level", "danger", "condition"]):
        intent = "area_risk"
        answer = (
        ...
        if pred['risk_category'] in ["Critical", "High"]:
            answer += f"🚨 **Advisory:** High risk detected! {pred['recommendations'][0]}"
        else:
            answer += "✅ **Advisory:** Conditions currently manageable. Monitor updates."
        ...
        actions.append({"label": "Find Shelters", "query": f"Which shelter should I use near {ward_name}?"})
        actions.append({"label": "Evacuation Route", "query": f"Which roads should I avoid in {ward_name}?"})

    # 2. Intent: Shelter Recommendation
    elif any(k in q_lower for k in ["shelter", "refuge", "camp", "stay", "medical", "relief"]):
        intent = "shelter_recommendation"
        needs_medical = "medical" in q_lower or "doctor" in q_lower
        needs_access = "wheelchair" in q_lower or "accessible" in q_lower or "disabled" in q_lower
        ...
        actions.append({"label": "Safe Route to Shelter", "query": f"Which roads should I avoid to get to {best.get('name')}?"})

    # 3. Intent: Roads to Avoid / Evacuation Route
    elif any(k in q_lower for k in ["road", "route", "avoid", "path", "drive", "navigate", "blocked", "evacuate"]):
        intent = "evacuation_routing"
        all_preds = get_all_ward_predictions()
        high_risk_wards = [f"w{w['ward_number']}" for w in all_preds if w['risk_score'] >= 60]
    ...
    # 4. Intent: Safety Precautions
    elif any(k in q_lower for k in ["precaution", "safety", "tip", "do", "prepare", "kit", "emergency"]):
        intent = "precautions"
        answer = (
        ...
        actions.append({"label": "Check Area Risk", "query": f"Is {ward_name} safe right now?"})

    else:
        intent = "overview"
        answer = (
            f"👋 **FloodGuard AI Assistant**: I am connected to Visakhapatnam GVMC live sensors & XGBoost models.\n"
            f"I can help you with:\n"
            f"• **Area Risk Status:** 'Is Gajuwaka safe?' or 'What is Ward 14 risk?'\n"
            f"• **Shelter Finder:** 'Which shelter should I use near MVP Colony?'\n"
            f"• **Road Blockages:** 'Which roads should I avoid in Ward 14?'\n"
            f"• **Safety Precautions:** 'What precautions should I take during heavy rain?'\n"
        )
        actions = [
            {"label": "Is Gajuwaka safe?", "query": "Is Gajuwaka safe right now?"},
            {"label": "Nearest Shelter", "query": "Which shelter should I use near Gajuwaka?"},
            {"label": "Roads to Avoid", "query": "Which roads should I avoid in Ward 14?"},
