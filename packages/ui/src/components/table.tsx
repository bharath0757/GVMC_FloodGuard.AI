...
const SortableHead: React.FC<SortableHeadProps> = ({ children, sortDirection, onSort, className, ...props }) => (
  <TableHead className={cn('cursor-pointer select-none hover:text-foreground', className)} onClick={onSort} {...props}>
    <div className="flex items-center space-x-1">
      ...
      {sortDirection === false && <ChevronsUpDown className="h-3.5 w-3.5 text-muted-foreground/60" />}
    </div>
  </TableHead>
);
...
const TablePagination: React.FC<TablePaginationProps> = ({
  ...
  <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-card">
    <div className="text-xs text-muted-foreground">
      {totalRecords !== undefined ? `Showing page ${currentPage} of ${totalPages} (${totalRecords} records)` : `Page ${currentPage} of ${totalPages}`}
    </div>
    <div className="flex items-center space-x-2">
      <button
        onClick={() => onPageChange(currentPage - 1)}
        disabled={currentPage <= 1}
        className="px-3 py-1 text-xs rounded border border-input disabled:opacity-40 hover:bg-muted transition-colors"
      >
        Previous
      </button>
      <button
...
export { Table, TableHeader, TableBody, TableFooter, TableHead, TableRow, TableCell, TableCaption, SortableHead, TablePagination };
