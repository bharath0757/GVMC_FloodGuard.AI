  ...
  'relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground transition-all duration-200',
  {
    variants: {
      variant: {
        ...
        success: 'bg-emerald-500/10 text-emerald-900 dark:text-emerald-200 border-emerald-500/30 [&>svg]:text-emerald-500',
        warning: 'bg-amber-500/10 text-amber-900 dark:text-amber-200 border-amber-500/30 [&>svg]:text-amber-500',
        danger: 'bg-red-500/10 text-red-900 dark:text-red-200 border-red-500/30 [&>svg]:text-red-500',
        emergency: 'bg-red-600 text-white border-red-700 font-medium shadow-lg [&>svg]:text-white animate-pulse',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>(
  ({ className, variant = 'default', icon, onDismiss, children, ...props }, ref) => {
    const selectedIcon = icon || defaultIcons[variant || 'default'];
    return (
      <div
        ref={ref}
        role={variant === 'emergency' || variant === 'danger' ? 'alert' : 'status'}
