...
const Drawer: React.FC<DrawerProps> = ({
  ...
  const positionClasses = {
    right:
      'inset-y-0 right-0 h-full w-full sm:w-[480px] border-l animate-in slide-in-from-right',
    left: 'inset-y-0 left-0 h-full w-full sm:w-[480px] border-r animate-in slide-in-from-left',
    bottom:
      'inset-x-0 bottom-0 max-h-[85vh] w-full border-t rounded-t-2xl animate-in slide-in-from-bottom',
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div
        ...
        aria-modal="true"
        className={cn(
          'bg-card border-border fixed z-50 flex flex-col justify-between overflow-y-auto p-6 shadow-2xl',
...
const DrawerHeader: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  <div
    className={cn('mb-6 flex flex-col space-y-1.5 pr-6', className)}
    {...props}
  />
);
...
const DrawerTitle: React.FC<React.HTMLAttributes<HTMLHeadingElement>> = ({
  ...
  <h2
    className={cn(
      'text-foreground text-xl font-bold leading-none tracking-tight',
      className,
    )}
    {...props}
  />
...
const DrawerFooter: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  <div
    className={cn(
      'border-border mt-auto flex flex-row justify-end space-x-2 border-t pt-6',
      className,
    )}
    {...props}
  />
